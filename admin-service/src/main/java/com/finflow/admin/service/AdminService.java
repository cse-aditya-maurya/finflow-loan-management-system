...
public class AdminService {
    ...
    // ===============================
    // 👁️ VIEW SINGLE DOCUMENT
    // ===============================
    public Object viewDocument(Long docId, String token) {
        return documentClient.viewDocument(docId, token);
    }
    ...
    // ===============================
    // 👤 GET USER BY ID WITH APPLICATIONS
    // ===============================
    public Map<String, Object> getUserById(Long userId, String token) {
        try {
            // Get user details from auth-service
            Object userObj = authClient.getUserById(token, userId);
            
            // Get all applications
            List<Map<String, Object>> allApps = applicationClient.getApplications(token);
            
            // Filter applications for this user
            List<Map<String, Object>> userApps = allApps.stream()
                .filter(app -> app != null && userId.equals(app.get("userId")))
                .filter(app -> !"DRAFT".equalsIgnoreCase(String.valueOf(app.get("status"))))
                .toList();
    ...
    public Map<String, Object> getStatistics(String token) {
        List<Map<String, Object>> apps = applicationClient.getApplications(token);
        ...
        return Map.of(
            "totalApplications", filtered.size(),
